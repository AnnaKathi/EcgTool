...
char ms_padd[4]; // 84, 0x54 byte record
...
#define MYSQL_COUNT_ERROR (~(my_ulonglong) 0)
	...
	enum mysql_option
		...
		/* MariaDB Connector/C specific */
			MYSQL_DATABASE_DRIVER = 7000, MARIADB_OPT_SSL_FP,
		/* single finger print for server certificate verification */
			MARIADB_OPT_SSL_FP_LIST,
		/* finger print white list for server certificate verification */
	...
	typedef struct st_mysql
		...
		/* changed from int to long in 4.1 protocol */
...
#include "my_stmt.h"
	...
	void STDCALL mysql_set_local_infile_handler(MYSQL *mysql,
		int(*local_infile_init)(void **, const char *, void *),
		int(*local_infile_read)(void *, char *, unsigned int),
		void(*local_infile_end)(void *), int(*local_infile_error)(void *, char*,
		unsigned int), void *);

	void mysql_set_local_infile_default(MYSQL *mysql);

	void my_set_error(MYSQL *mysql, unsigned int error_nr, const char *sqlstate,
		const char *format, ...);
...
#define mysql_debug_init(A) mysql_debug((A));
	...
	size_t STDCALL mariadb_convert_string(const char *from, size_t *from_len,
		CHARSET_INFO *from_cs, char *to, size_t *to_len, CHARSET_INFO *to_cs,
		int *errorcode);
	...
	int STDCALL mysql_stmt_next_result_cont(int *ret, MYSQL_STMT *stmt,
		int status);

	int STDCALL mysql_stmt_close_start(my_bool *ret, MYSQL_STMT *stmt);
	int STDCALL mysql_stmt_close_cont(my_bool *ret, MYSQL_STMT * stmt,
		int status);
	...
	int STDCALL mysql_change_user_cont(my_bool *ret, MYSQL *mysql, int status);
	...
	int STDCALL mysql_query_start(int *ret, MYSQL *mysql, const char *q);
	int STDCALL mysql_query_cont(int *ret, MYSQL *mysql, int status);
	int STDCALL mysql_send_query_start(int *ret, MYSQL *mysql, const char *q,
		unsigned long length);
	int STDCALL mysql_send_query_cont(int *ret, MYSQL *mysql, int status);
	int STDCALL mysql_real_query_start(int *ret, MYSQL *mysql, const char *q,
		unsigned long length);
	...
	int STDCALL mysql_shutdown_start(int *ret, MYSQL *mysql,
		enum mysql_enum_shutdown_level shutdown_level);
	int STDCALL mysql_shutdown_cont(int *ret, MYSQL *mysql, int status);
	...
	int STDCALL mysql_stat_cont(const char **ret, MYSQL *mysql, int status);
	...
	int STDCALL mysql_read_query_result_cont(my_bool *ret, MYSQL *mysql,
		int status);
	...
	int STDCALL mysql_stmt_reset_cont(my_bool *ret, MYSQL_STMT *stmt,
		int status);
	...
	int STDCALL mysql_stmt_send_long_data_start(my_bool *ret, MYSQL_STMT *stmt,
		unsigned int param_number, const char *data, unsigned long len);
	...
	struct st_mysql_methods
		...
		int(*db_command)(MYSQL * mysql, enum enum_server_command command,
			const char *arg, size_t length, my_bool skipp_check, void *opt_arg);
		void(*db_skip_result)(MYSQL * mysql);
		int(*db_read_query_result)(MYSQL * mysql);
		MYSQL_DATA *(*db_read_rows)(MYSQL *mysql, MYSQL_FIELD *fields,
			unsigned int field_count);
		int(*db_read_one_row)(MYSQL * mysql, unsigned int fields, MYSQL_ROW row,
