...
double *feature_min;
//>> Anpassung Anna >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//DBL_MAX ist unbekannt obwohl das eig in float.h angelegt wurde
//tatsächliche Zahl eintragen
double y_max = -1.79769e+308; //replaced: -DBL_MAX;
double y_min =  1.79769e+308; //replaced:  DBL_MAX;
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
int max_index;
int min_index;
